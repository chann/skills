...
- 한국어 보고서는 자연스럽고 구체적으로 쓰며, `human-friendly-writing`이 이미 있으면 마지막에 한 번 더 다듬음
- 기본은 대화 응답, 파일은 요청 시에만 생성하고 커밋하지 않음

`work-summary`만 설치해도 모든 기능을 쓸 수 있습니다.
`human-friendly-writing`을 설치하거나 요구하지 않으며, 사용할 수 없을 때는
스킬에 포함된 문체 규칙만으로 보고서를 완성합니다.
