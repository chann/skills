...
| `handoff` | 0.3.0 | `gen-frontend-handoff`, `gen-backend-handoff`, `gen-session-handoff` | Generate evidence-based continuation handoffs for frontend/client and backend/server developers from diffs, ranges, branch comparisons, and session context, plus session-to-session handoffs that split proven state from unproven |
...
- **`handoff`** keeps both handoff generators as self-contained `SKILL.md` files. `gen-frontend-handoff` focuses on client-visible API contract changes, type/rendering/error-state work, and `client action 없음` for DB-only changes. `gen-backend-handoff` focuses on API contracts, database migrations, jobs/queues, rollout, verification, and backend continuation prompts. `gen-session-handoff` hands the session itself rather than a diff: it splits proven state from unproven with the exact command behind each proven claim, references plans, specs, reviews, and diagnosis records by path instead of restating them, records decisions with their reasons alongside the approaches ruled out, and closes with ordered next actions carrying done-checks plus a copyable resume prompt.
