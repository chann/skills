...
SKILL = DOC_SKILL / "skills" / "doc-gen" / "SKILL.md"


class DocSkillPackageTests(unittest.TestCase):
    def test_skills_cli_discovers_doc_gen(self) -> None:
        env = os.environ.copy()
        ...
        self.assertRegex(result.stdout, re.compile(r"(?m)^[^A-Za-z0-9]*doc-gen\s*$"))
    ...
    def test_doc_skill_plugin_shape_is_packaged(self) -> None:
        ...
        for name in [
            ...
            with self.subTest(template=name):
                self.assertTrue((DOC_SKILL / "skills" / "doc-gen" / "templates" / name).is_file())

        for name in ["README.md", "README.ko.md", "ARCHITECTURE.md", "USAGE.md"]:
            with self.subTest(doc=name):
                self.assertTrue((DOC_SKILL / name).is_file())

    def test_doc_skill_metadata_and_command_route_to_doc_gen(self) -> None:
        ...
        self.assertIn("Use the **doc-gen** skill", command)
        self.assertIn("README.md", command)
        self.assertIn("ARCHITECTURE.md", command)
        self.assertIn("USAGE.md", command)

    def test_doc_gen_skill_enforces_document_contracts_and_safety(self) -> None:
    ...
    def test_templates_encode_the_four_doc_roles(self) -> None:
        templates = DOC_SKILL / "skills" / "doc-gen" / "templates"
